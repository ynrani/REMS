...
<body>
	<div class="wrapper mainAll">
		...
		<div class="container">
			...
			<div class="two-col">
				<form:form id="remsEnvBookingListingForm"
					name="remsEnvBookingListingForm"
					action="${pageContext.request.contextPath}/remsEnvBookingListing"
 			...
 			%>
							<div class="nav" id="myid">
									...
									<tbody>
										<c:forEach items="${remsEnvBookingsDTOs}"
											...
											<tr>
												...
												<td align="left"><c:if
														test="${remsEnvBookingsDTOs.status ne 'Confirm'}">
														<a class="hrefVisited" href="#"
															onClick="deleteBooking(${remsEnvBookingsDTOs.id});">Delete</a>
													</c:if> <c:if test="${remsEnvBookingsDTOs.status eq 'Confirm'}">
														<a class="hrefVisited not-active"
															title="Confirmed bookings cannot be deleted...">Delete</a>
													</c:if></td>
												</div>
											</tr>
										</c:forEach>
								...
								<table style="width: 100%; border: 0">
									<tbody>
										<tr>
											<th scope="col" class="buttonsAll15"><input
...
window.location.hash = "myid";
   ...
   function deleteBooking(reqId)
   {
	   confirmation_dialogYNCancelBooking(reqId,'Submit','Are you sure do you want to cancel the request ?',400,'YesNo','Yes','No');
   }
