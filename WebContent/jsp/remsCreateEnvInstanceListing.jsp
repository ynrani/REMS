...
<body>
	<div class="wrapper mainAll">
		...
		<div class="container">
			...
			<div class="two-col">
				<form:form id="remsCreateEnvInstanceListingForm"
					name="remsCreateEnvInstanceListingForm"
					action="${pageContext.request.contextPath}/remsCreateEnvInstanceListing"
 			...
 			%>
										...
										<c:forEach items="${remsCreateEnvInstanceDTOs}"
											...
											<tr>
												...
												<td align="left"><a class="hrefVisited" href="#"
													onClick="deleteInstance(${remsCreateEnvInstanceDTOs.id});">Delete</a>
												</td>
											</tr>
										</c:forEach>
									</tbody>
							...
							<ul class="grdPagination">
								<%
											...
											if (currentPage != 1) {
			   							%>
								<li><a href="remsCreateEnvInstanceListing?page=<%= 1 %>">First</a>
									<div>First</div></li>
								<li><a
									href="remsCreateEnvInstanceListing?page=<%= currentPage-1 %>">&lt;
										Prev</a>
									<div>&lt; Prev</div> <%
			   								} else {
			   								 	if(noOfPages > 1) {
			   							%>
								<li class="disable"><a
									href="remsCreateEnvInstanceListing?page=<%= 1 %>">First</a>
									<div>First</div></li>
								<li class="disable"><a
									href="remsCreateEnvInstanceListing?page=<%= currentPage-1 %>">&lt;
										Prev</a>
								...
								<%
			    						...
			    						%>
								<li><a href="remsCreateEnvInstanceListing?page=<%= i %>"
									id="employeeLink"><%= i %></a>
									<div><%= i %></div></li>
								<%
			    									}
										...
										%>
								<li><a
									href="remsCreateEnvInstanceListing?page=<%= currentPage+1 %>">Next
										&gt;</a>
									<div>Next &gt;</div></li>
								<li><a
									href="remsCreateEnvInstanceListing?page=<%= noOfPages %>">Last</a>
									<div>Last</div></li>
								<%
											} else {
											    if(noOfPages > 1) {
...
window.location.hash = "myid";
   ...
   function deleteInstance(reqId)
   {
	   confirmation_dialogYNCancelInstanceServer(reqId,'Submit','Are you sure do you want to cancel the request ?',400,'YesNo','Yes','No');
   }
