...
<body>
	<div class="wrapper mainAll">
		...
		<div class="container">
			...
			<div class="two-col">
				<form:form id="remsCreateHostServerListingForm"
					name="remsCreateHostServerListingForm"
					action="${pageContext.request.contextPath}/remsCreateHostServerListing"
						...
						<tbody>
							...
							<tr>
								...
								<td class="flied-title" align="left" valign="middle"><form:input
										path="ip" id="ip" class="form-control" /></td>
 			...
 			%>
										...
										<c:forEach items="${remsCreateHostServerDTOs}"
											...
											<tr>
												...
												<td align="left"><a class="hrefVisited" href="#"
													onClick="deleteServer(${remsCreateHostServerDTOs.id});">Delete</a>
												</td>
											</tr>
										</c:forEach>
									</tbody>
							...
							<ul class="grdPagination">
								<%
											...
											if (currentPage != 1) {
			   							%>
								<li><a href="remsCreateHostServerListing?page=<%= 1 %>">First</a>
									<div>First</div></li>
								<li><a
									href="remsCreateHostServerListing?page=<%= currentPage-1 %>">&lt;
										Prev</a>
									<div>&lt; Prev</div> <%
			   								} else {
			   								 	if(noOfPages > 1) {
			   							%>
								<li class="disable"><a
									href="remsCreateHostServerListing?page=<%= 1 %>">First</a>
									<div>First</div></li>
								<li class="disable"><a
									href="remsCreateHostServerListing?page=<%= currentPage-1 %>">&lt;
										Prev</a>
								...
								<%
			    						...
			    						%>
								<li><a href="remsCreateHostServerListing?page=<%= i %>"
									id="employeeLink"><%= i %></a>
									<div><%= i %></div></li>
								<%
			    									}
										...
										%>
								<li><a
									href="remsCreateHostServerListing?page=<%= currentPage+1 %>">Next
										&gt;</a>
									<div>Next &gt;</div></li>
								<li><a
									href="remsCreateHostServerListing?page=<%= noOfPages %>">Last</a>
									<div>Last</div></li>
								<%
											} else {
											    if(noOfPages > 1) {
...
window.location.hash = "myid";
   ...
   function deleteServer(reqId)
   {
	   confirmation_dialogYNCancelHostServer(reqId,'Delete','Are you sure do you want to delete the Server ?',400,'YesNo','Yes','No');
   }
