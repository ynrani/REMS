...
import com.rems.model.DTO.ValidationResponse;
 ...
 */
@Controller
public class RemsAdminController
{
	private static Logger logger = Logger.getLogger(RemsAdminController.class);

	@Resource(name = MessageConstant.SERVICE_ADMIN)
	RemsAdminService remsAdminService;
	...
	public String userDetails(@ModelAttribute(MessageConstant.USER_DO) TdmUserDTO userdo,
		...
		try {
			...
			String strMessage = remsAdminService.saveUserDetails(userdo, bEdit);
	...
	public String displayUser(
		...
		try {
			...
			totalRecords = remsAdminService.searchUserRecordsCount(userdo);
			List<TdmUserDTO> DiaplayUser = remsAdminService.getAllUser(userdo, offSet,
					recordsperpage, true);
	...
	public String editUser(@ModelAttribute(MessageConstant.USER_DO) TdmUserDTO userdo,
		...
		try {
			...
			userdo = remsAdminService.getEditUser(userId);
	...
	public String daleteUser(@ModelAttribute(MessageConstant.USER_DO) TdmUserDTO userdo,
		...
		logger.info(MessageConstant.TDM_ADMIN_CTRL + MessageConstant.TDM_ADMIN_DELETE_USER
				+ MessageConstant.LOG_INFO_PARAMS_NO);
		try {
			remsAdminService.deleteUserByUserId(delUserId);
			logger.info(MessageConstant.TDM_ADMIN_CTRL + MessageConstant.TDM_ADMIN_DELETE_USER
					+ MessageConstant.LOG_INFO_RETURN);
	...
	public @ResponseBody ValidationResponse validateUserId(
		...
		try {
			...
			boolean bCheck = remsAdminService.validateUserId(userid);
			if (!bCheck) {
