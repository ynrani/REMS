...
public class RemsReportsExcelView extends AbstractExcelView
{

	@Override
	protected void buildExcelDocument(Map<String, Object> model, HSSFWorkbook workbook, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		HSSFSheet excelSheet = workbook.createSheet("Customer User Report");
	...
	public void setExcelHeader(HSSFSheet excelSheet, HSSFCellStyle style) {
		...
		excelHeader.createCell(4).setCellValue("Environment User");
		excelHeader.createCell(5).setCellValue("Environment Booked to");
		excelHeader.createCell(6).setCellValue("Environment Connectivity");
		excelHeader.createCell(7).setCellValue("Booking Start Date");
		excelHeader.createCell(8).setCellValue("Booking End Date");
	...
	public void setExcelRows(HSSFSheet excelSheet, List<RemsEnvBookingsDTO> remsEnvBookingsDTOs, HSSFCellStyle style) {
		...
		for (RemsEnvBookingsDTO remsEnvBookingsDTO : remsEnvBookingsDTOs) {
			...
			excelRow.createCell(1).setCellValue(remsEnvBookingsDTO.getUserId());
			excelRow.createCell(2).setCellValue(remsEnvBookingsDTO.getEnvName());
			excelRow.createCell(3).setCellValue(remsEnvBookingsDTO.getAppName());
			excelRow.createCell(4).setCellValue(remsEnvBookingsDTO.getEnvUser());
			excelRow.createCell(5).setCellValue(remsEnvBookingsDTO.getBookedTo());
			excelRow.createCell(6).setCellValue(remsEnvBookingsDTO.getEnvConn());
			excelRow.createCell(7).setCellValue(remsEnvBookingsDTO.getStartDt());
			excelRow.createCell(8).setCellValue(remsEnvBookingsDTO.getEndDt());
