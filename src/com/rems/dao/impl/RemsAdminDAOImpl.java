/*---------------------------------------------------------------------------------------
 * Object Name: RemsAdminDAOImpl.Java
 * 
 * Modification Block:
 * --------------------------------------------------------------------------------------
...
import com.rems.model.DO.TdmUsersAuthDOPK;
...
public class RemsAdminDAOImpl implements RemsAdminDAO
{
	private static Logger logger = Logger.getLogger(RemsAdminDAOImpl.class);

	@Override
	public String saveUserDetails(TdmUserDO userdo, boolean bCreate, EntityManager managerUser) throws DAOException {
		...
		try {
			...
			String idrole = "";
			if (bCreate) {
				String no = (String) managerUser.createQuery("SELECT MAX(p.id.userRoleId)  from TdmUsersAuthDO p")
						.getSingleResult();
	...
	public List<TdmUserDO> getAllUser(TdmUserDO userdo, int offSet, int recordsperpage, boolean b,
		...
		try {
			...
			List<TdmUserDO> listUser = managerUser
					.createQuery("SELECT p FROM TdmUserDO p where p.userId != '" + userdo.getUserId() + "' ")
	...
	public String deleteUserByUserId(String userId, EntityManager managerUser) throws DAOException {
		...
		try {
			managerUser.getTransaction().begin();
			Query q1 = managerUser.createQuery("DELETE FROM  TdmUsersAuthDO p where p.id.userId =:userId");
	...
	@Override
	public Long searchUserRecordsCount(TdmUserDO userdo, EntityManager managerUser) throws DAOException {
	...
	public boolean validateUserId(String userId, EntityManager managerUser) throws DAOException {
		logger.info(MessageConstant.TDM_ADMIN_DAO_IMPL + MessageConstant.TDM_ADMIN_DAO_VALIDATE
				+ MessageConstant.LOG_INFO_PARAMS_NO);
		try {
			List<TdmUserDO> listUserDo = managerUser.createNamedQuery("TdmUserDO.findByUserId", TdmUserDO.class)
					.setParameter("userId", userId).getResultList();
			logger.info(MessageConstant.TDM_ADMIN_DAO_IMPL + MessageConstant.TDM_ADMIN_DAO_VALIDATE
					+ MessageConstant.LOG_INFO_RETURN);
			return listUserDo != null && !listUserDo.isEmpty() ? false : true;
		} catch (IllegalStateException illegalStateEx) {
			logger.error(MessageConstant.TDM_ADMIN_DAO_IMPL + MessageConstant.TDM_ADMIN_DAO_VALIDATE
					+ MessageConstant.LOG_ERROR_EXCEPTION);
			throw new DAOException(MessageConstant.NRE_ENTITY_MGR_FACTORY_CLOSED_EXCEPTION, illegalStateEx);
		} catch (IllegalArgumentException illegalArgEx) {
			logger.error(MessageConstant.TDM_ADMIN_DAO_IMPL + MessageConstant.TDM_ADMIN_DAO_VALIDATE
	...
	public String getUserRole(TdmUserDO userdo, EntityManager managerUser) throws DAOException {
		...
		try {
			...
			List<TdmUserDO> listUserAuthDo = managerUser.createQuery(
					" select p from TdmUserDO p where p.userId = '" + userdo.getUserId() + "' ").getResultList();
			logger.info(MessageConstant.TDM_ADMIN_DAO_IMPL + MessageConstant.TDM_ADMIN_DAO_GET_ALL
					+ MessageConstant.LOG_INFO_RETURN);
			if (listUserAuthDo != null && !listUserAuthDo.isEmpty())
	...
	public TdmUserDO checkAvailabilityOfUserId(String userId, EntityManager managerUser) throws DAOException {
		...
		try {
			...
			logger.error(MessageConstant.TDM_ADMIN_DAO_IMPL + MessageConstant.TDM_FTD_CHECK_AVIL_USER
					+ MessageConstant.LOG_ERROR_EXCEPTION);
			throw new DAOException(MessageConstant.NRE_ENTITY_MGR_FACTORY_CLOSED_EXCEPTION, illegalStateEx);
		} catch (IllegalArgumentException illegalArgEx) {
			logger.error(MessageConstant.TDM_ADMIN_DAO_IMPL + MessageConstant.TDM_FTD_CHECK_AVIL_USER
